 -- 1.1 Tempo de uso da reserva

-- Deve retornar 2 (14h → 16h)
SELECT fn_tempo_uso(1) AS horas_reserva1;

-- Deve retornar 1 (10h → 11h30)
SELECT fn_tempo_uso(2) AS horas_reserva2;


-- 1.2 Verificar se usuário está punido

-- Usuário 1 (tem punição ativa)
SELECT fn_usuario_punido(1) AS punido1;

-- Usuário 2 (punição já resolvida)
SELECT fn_usuario_punido(2) AS punido2;

-- Usuário 4 (sem punição)
SELECT fn_usuario_punido(4) AS punido4;


-- 2. Testar Procedures
-- 2.1 Listar reservas de um usuário
CALL sp_listar_reservas_usuario(1);
CALL sp_listar_reservas_usuario(2);

-- 2.2 Criar reserva (cenários diferentes)
...
-- 3.2 Cancelamento de reserva aprovada (restaura quantidade)
...
-- 4. Consultas Extras de Validação
